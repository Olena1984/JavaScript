"use strict";
const listSitesDevelopedCompany = [
    {
        title: "Rozetka",
...
let totalCostsSites = listSitesDevelopedCompany.reduce((prevCost, currentCost) => prevCost + currentCost.costOfSite, 0);
console.log(totalCostsSites);
document.write(`<p>Загальна вартість усіх сайтів: ${totalCostsSites}</p><br>`);
// 2) кількість сайтів, що було зроблено між 2000 та 2009 рр.
let sitesCount = listSitesDevelopedCompany.filter((element) => element.productionYear >= 2000 && element.productionYear <= 2009).length;
...
document.write(`<p>Кількість сайтів, що було зроблено між 2000 та 2009 рр = ${sitesCount}</p><br>`);
// 3) кількість сайтів, де сума спонсорських вкладень була більшою за 100000
let sponsorsSumMore10000 = listSitesDevelopedCompany.filter((element) => {
    let sponsorsSum = element.sponsors.reduce((prevSum, sponsor) => prevSum + sponsor.sponsorsInvestments, 0);
    return sponsorsSum > 100000;
}).length;
console.log(sponsorsSumMore10000);
document.write(`<p> Кількість сайтів, де сума спонсорських вкладень була більшою за 100000 = ${sponsorsSumMore10000}</p><br>`);
...
let yearMaxProduct = listSitesDevelopedCompany.reduce((maxProduct, currentProduct) => maxProduct.costOfSite > currentProduct.costOfSite
    ? maxProduct
    : currentProduct);
console.log(yearMaxProduct.productionYear);
document.write(`<p>Рік, коли прибуток був найбільшим = ${yearMaxProduct.productionYear}</p><br>`);
// 6) упорядкувати список за спаданням прибутку
let sortProductList = [...listSitesDevelopedCompany].sort((a, b) => b.costOfSite - a.costOfSite);
