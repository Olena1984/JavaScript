...
class TMoney {
  #sum$: number = 0;
  #dollarRate: number = 45;
    #newRate$:number =0
  constructor(sum$: number = 0, dollarRate: number = 45) {
    this.Sum$ = sum$;
  ...
  set Sum$(newSum: number) {
    this.#sum$ = newSum;
  }
  get DollarRate() {
    return this.#dollarRate;
  }
  set DollarRate(newRate:number) {
    this.#dollarRate =newRate;
  }
  addMoney(moneyGrn: number) {
    const money$ = +(moneyGrn / this.#dollarRate).toFixed(2);
    this.#sum$ += money$;
    return this.#sum$;
  }
  removeMoney(moneyGrn: number) {
