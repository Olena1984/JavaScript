...
let pupilsAge: number[] = [12, 10, 6, 9, 8, 13, 10, 15, 12, 9, 8];
...
document.write(
  `Максимальний вік = ${maxAge}. Зустрічається ${map.get(maxAge)} раз`,
);
document.write(`<hr>`);
// ===================================================================================================
...
for (const temperature of temperatures) {
  if (tempMap.has(Math.ceil(temperature))) {
    tempMap.set(
      ...
      tempMap.get(Math.ceil(temperature)) + 1,
    );
  } else {
