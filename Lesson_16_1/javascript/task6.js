...
class Company {
    #company = "";
    #dateOfEstablishment = [0, 0];
    #services = [];
    #branchAddresses = [];
    #fullYears = 0;
    #city = '';
    #currentServices = '';
    constructor(company, dateOfEstablishment, services, branchAddresses) {
    ...
    displayBranches(city) {
        if (this.BranchAddresses) {
            ...
            // або
            const findBranches = this.BranchAddresses.flatMap((element) => element.branch).filter((element) => element.city === city);
            this.#city = findBranches.map(element => `${element.city}, ${element.street}, ${element.houseNumber}`).join('\n');
        }
        return this.#city;
    }
    // виведення на екран послуг, що можуть бути виконані за вказану суму грошей та вказаний термін часу;
    displayServices(cost, year) {
        if (this.Services) {
...
const dateEstablishment = [2005, 7];
const branchAddr = [
    {
        country: "Canada",
        branch: [
