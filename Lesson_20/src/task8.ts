...
class SelectForm {
  ...
  render() {
    ...
    if (container) container.append(this.selectElement);
  }
}
...
class Choice {
  ...
  constructor(
    public array: Auto[],
    public brandAuto: SelectForm,
    public year: SelectForm,
    public price: SelectForm,
  ) {
  ...
  selectItem() {
    ...
    const findSelect = () => {
      ...
      const filterItem = this.array.filter(
        (item) =>
       ( brandValue!.value === "" || item.brand === brandValue!.value) &&
  ...
  selected() {
    const divEl = document.querySelector(".select-items") as HTMLDivElement;
    divEl.onchange = (event: Event) => {
  ...
  render() {
    ...
    if (container) container.append(this.displayList());

    this.selectItem();
    this.selected();
  }
}
const arrayData: Auto[] = [
  {
    brand: "BMW",
    year: 2000,
    price: 45000,
...
window.onload = function () {
  ...
  const choiceOptions = new Choice(
    arrayData,
    selectBrand,
    selectYear,
    selectPrice,
