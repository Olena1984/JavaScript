...
class Lists {
    ...
    render() {
        const container = document.querySelector('.search-container');
        if (container)
            container.append(this.listElement);
    }
}
class Search {
    inputForm;
    list;
    constructor(inputForm, list) {
        this.inputForm = inputForm;
        this.list = list;
    }
    searchMatch() {
        const inputEl = this.inputForm.inputElement.querySelector('input');
        inputEl.oninput = (oninput) => {
            const targetEl = oninput.target;
...
const list = ["Бокало М.С", "Онисько Б.Б", "Степанців І.Ю.", "Гончаренко Г.О.", "Бокало В.О.", "Онищенко К.Г"];
