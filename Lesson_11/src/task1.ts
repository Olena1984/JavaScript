...
function getSimpleArray(column: number, max: number, min: number): number[] {
  let arr: number[] = [];
...
document.write(
  `1.Сума елементів першої половини рядків та стовпців = ${sum1}<br>`,
);
console.log(sum1);
// reduce
let sumArea1 = multidimensionalArray.reduce(
  (previousSum: number, rows: number[], rowIndex: number) => {
    if (rowIndex < halfRow) {
      return (
        previousSum +
        rows.reduce((prevValue: number, cols: number, colIndex: number) => {
          if (colIndex < halfCol) {
...
document.write(
  `2.Сума елементів першої половини рядків та другої половини стовпців = ${sum2}<br>`,
);
console.log(sum2);

// reduce
let sumArea2 = multidimensionalArray.reduce(
  (previousSum: number, rows: number[], rowInd: number) => {
    if (rowInd < halfRow) {
      return (
        previousSum +
        rows.reduce((previousValue: number, cols: number, colInd: number) => {
          if (colInd >= halfCol) {
...
document.write(
  `3.Сума елементів другої половини рядків та першої половини стовпців = ${sum3}<br>`,
);
console.log(sum3);
// reduce
let sumArea3 = multidimensionalArray.reduce(
  (previousSum: number, rows: number[], rowInd: number) => {
    if (rowInd >= halfRow) {
      return (
        previousSum +
        rows.reduce((colsSum: number, cols: number, colInd: number) => {
          if (colInd < halfCol) {
            return colsSum + cols;
          } else {
            return colsSum;
...
document.write(
  `4.Сума елементів другої половини рядків та другої половини стовпців = ${sum4}<br>`,
);
console.log(sum4);
// reduce
let sumArea4 = multidimensionalArray.reduce(
  (previousSum: number, rows: number[], rowInd: number) => {
    if (rowInd >= halfRow) {
      return (
        previousSum +
...
// reduce
let sumEvenRows = multidimensionalArray.reduce(
  (prevSum: number, rows: number[], rowInd: number) => {
    if (rowInd % 2 === 0) {
      return (
        prevSum +
        rows.reduce((colSum: number, cols: number) => colSum + cols, 0)
      );
    } else {
      return prevSum;
    }
...
// reduce
let oddSum = multidimensionalArray.reduce((persSum: number, rows: number[]) => {
  return (
    persSum +
...
document.write(
  `7.Сума непарних стовпців у парних рядках = ${sumEvenRoddC}, а парних елементів у непарних стовпцях = ${sumOddRevenC}`,
);
