...
let gameField = Array.from(
  { length: rows },
  () => Array.from({ length: cols }).fill(0) as number[],
);
let shipNum = 5;
let shipPlace = 0;
while (shipPlace < shipNum) {
  let randRow: number = Math.floor(Math.random() * gameField.length);
  let randCol: number = Math.floor(Math.random() * gameField[0].length);
...
function seaBattle(gameField: number[][], ships: number): string {
  ...
  while (bullets > 0 && ships > 0) {
    ...
    if (userShotRow === null || userShotCol === null) return "Гра скасована";
    ...
    if (
      ...
      if (gameField[correctShotRow][correctShotCol] === 1) {
        ...
        bullets--;
        alert(
          `Постріл мимо (або повторний постріл) - ваші снаряди зменшилися на 1`,
        );
      }
    }
  }
