...
const row = 10;
const col = 7;
let arrTable = Array.from({ length: row }, () => Array.from({ length: col }, () => Math.floor(min + Math.random() * (max - min) + 1)));
const profitsArr = arrTable.map((row) => row.join(" , ")).join("<br>");
document.write(`<p>Масив ${row} магазинів з прибутком за ${col} днів:<br><br>${profitsArr}<br><br></p>`);
console.log(arrTable);
//  1) загальний прибуток кожного магазину за тиждень;
// for
let totalProfitEveryShop = [];
...
//   [],
// );
document.write(`<p>1.Загальний прибуток кожного магазину за тиждень ${totalProfitEveryShop}</p><br>`);
console.log(totalProfitEveryShop);
// 2) загальний прибуток усіх магазинів по днях (загальний прибуток усіх магазинів за понеділок, за вівторок, і т.д.);
// for
let totalProfitForDay = [];
...
document.write(`<p>2. Прибуток магагинів по днях: ${totalProfitForDay}</p><br>`);
...
document.write(`<p>3.Загальний прибуток за робочі дні: ${totalSumWorkDay}</p><br>`);
...
document.write(`<p>4.Загальний прибуток за вихідні дні: ${sumForRastDays}</p><br>`);
console.log(sumForRastDays);
// 5) максимальний прибуток за середу
let maxProfitOfWednesday = -Infinity;
for (let shops = 0; shops < arrTable.length; shops++) {
    const wednesday = arrTable[shops][2];
    if (wednesday > maxProfitOfWednesday)
        maxProfitOfWednesday = wednesday;
}
// reduce
...
document.write(`<p>5.Максимальний прибуток за середу: ${maxProfitOfWednesday}</p><br>`);
...
document.write(`<p>6.Список (одновимірний масив) зі значеннями, які більші за 200: ${profitMore200}</p><br>`);
console.log(profitMore200);
// 7) відсортувати кожен тиждень за зростанням
let newArrTable = JSON.parse(JSON.stringify(arrTable));
for (let shops = 0; shops < newArrTable.length; shops++) {
    newArrTable[shops].sort((a, b) => {
        if (a > b)
            return 1;
        else if (a < b)
            return -1;
...
document.write(`<p>7.Кожен тиждень відсортований за зростанням:<br> ${joinedArray}</p><br>`);
// console.log(arrTable);
...
let sortArr = arrTable.sort((a, b) => {
    ...
    // }
    let maxA = a.reduce((prevMax, number) => (prevMax > number ? prevMax : number), 0);
    let maxB = b.reduce((prevMax, number) => (prevMax > number ? prevMax : number), 0);
    if (maxB > maxA)
        return 1;
    if (maxA > maxB)
        return -1;
...
document.write(`<p>8.Відсортовані тижні (рядки) за спаданням максимального елемента у цьому тижні (рядку):<br> ${splitSortArr}</p><br>`);
console.log(sortArr);
...
// 9) упорядкувати тижні (рядки) за спаданням суми елементів у рядку (тобто при порівнянні двох рядків треба знайти суму кожного з рядків і порівнювати ці суми, на основі цих сум буде зрозуміло, який з елементів повинен іти раніше).
let arrTableNew = JSON.parse(JSON.stringify(arrTable));
let sumSortArr = arrTableNew.sort((a, b) => {
    let sumA = a.reduce((prevSum, number) => prevSum + number, 0);
    let sumB = b.reduce((prevSum, num) => prevSum + num, 0);
    if (sumA > sumB)
        return -1;
    else if (sumA < sumB)
        return 1;
...
document.write(`<p>9.Упорядкувані тижні (рядки) за спаданням суми елементів у рядку:<br> ${splitSumSortArr}</p><br>`);
