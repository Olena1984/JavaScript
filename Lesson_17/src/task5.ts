...
class RandNumbers {
  constructor(
    public numMin: number,
    public numMax: number,
  ) {}
  getRandNum() {
    return Math.floor(
      this.numMin + Math.random() * (this.numMax - this.numMin + 1),
    );
  }
}
...
class TestManager {
    history:HistoryTest = new HistoryTest()
    static manager:TestManager|null
  constructor(
    public multiply: MultiChecker,
    public adding: AddChecker,
    public timer: number,
  ...
  generateTask() {
    ...
    if (randIndTask > 0.5) {
        ...
        const userAnswer:number = parseInt(prompt(`${this.adding.num1}+${this.adding.num2} = `,'0')!)
    ...
    if(randIndTask<0.5){
        ...
        const userAnswer:number = parseInt(prompt(`${this.multiply.num1}*${this.multiply.num2} = `,'0')!)
