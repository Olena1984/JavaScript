...
function getVisitorNumber(quantityOfDays: number): number[] {
  let visitor: number[] = [];
  let day: number;
...
function countingVisitors(quantityVisitors: number[]): string {
  let less20: number[] = [];
  for (let i = 0; i < 7; i++) {
...
function minMaxQuantityVisitors(quantity: number[]): string {
  ...
  let minDaysVisitors: number[] = [];
  let maxDaysVisitors: number[] = [];
...
function totalVisitor(quantity: number[]): string {
  let totalVisitorInWorkDay: number = 0;
  let totalVisitorInRestDay: number = 0;
