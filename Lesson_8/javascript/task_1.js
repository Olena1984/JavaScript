...
function getArrayGrades(numOfSubjects) {
    let grades = [];
...
function assignCategory(number) {
    let category = "";
    if (number >= 4.75 && number <= 5.0)
        category = "excellent student";
    if (number >= 3.75 && number < 4.75)
        category = "good student";
    if (number >= 3.0 && number < 3.75)
        category = "satisfactory student";
    if (number < 3.0)
        category = "bad student";
    return category;
}
// вивід результатів
