...
function userBuyList(prices, products, money) {
    let boughtPrices = [];
    let boughtTitles = [];
    let sum = 0;
    let balanceMoney = money;
...
const { boughtPrices, boughtTitles, balanceMoney } = userBuyList(pricesProducts, titlesProducts, userMoney);
document.write(`<div>Список товарів ${titlesProducts}.<br> Ціни на товари ${pricesProducts}.<br> Кошти покупця ${userMoney} грн.<br> Покупець купив наступні товари: ${boughtTitles}<br>за такими цінами: ${boughtPrices}.<br>Залишок коштів = ${balanceMoney}грн.</div>`);
