...
let countClick:number[]= [0, 0, 0]
parentEl.onclick = function (event: Event) {
  const cell = event.target as HTMLTableCellElement;
  let parentTable = cell.closest("table") as HTMLTableElement;
  const pElement = parentTable.nextElementSibling as HTMLElement;
  const span = pElement.firstElementChild as HTMLSpanElement;
  if (cell.tagName === "TD") {
