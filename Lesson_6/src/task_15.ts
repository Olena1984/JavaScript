...
let userAttemptRow: number;
let userAttemptCol: number;
...
do {
  ...
  if (userAttemptRow === rowPosRabbit && userAttemptCol === colPosRabbit) {
    win = true;
    alert(
      `Вітаю ви влучили у зайця рядок ${rowPosRabbit} стовпець ${colPosRabbit}. Кількість витрачених куль ${bullets}`,
    );
    break;
  } else if (bullets === 0) {
    alert(
      `Спроби вичерпано. Заяць знаходився в рядку ${rowPosRabbit} і стовпці ${colPosRabbit}. Дякую за гру.`,
    );
    break;
  } else {
    ...
    if (rowPosRabbit < 1) rowPosRabbit = 1;
    if (rowPosRabbit > 10) rowPosRabbit = 10;
    if (colPosRabbit < 1) colPosRabbit = 1;
    if (colPosRabbit > 10) colPosRabbit = 10;
...
if (win) {
  document.write(
    `<p>Вітаю ви знайшли зайця. Заяць заховався в рядку ${rowPosRabbit} і стовпці ${colPosRabbit}</p>`,
  );
} else {
  document.write(
    `<p> Не вгадав. Заяць знаходився в рядку ${rowPosRabbit} і стовпці ${colPosRabbit}</p>`,
  );
}
