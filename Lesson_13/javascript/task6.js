...
*/
const givenWord = "summer";
function iteratorWords(word) {
    let indexWord = 0;
...
// гра перекладач
alert("Загадане слово асоціюється переважно з літом. Цей напій полюбляють як діти так і дорослі. Охолоджує у жарку погоду.\nЗдогоадалися,що це?\nПідказка слово містить 8 літер(розгадка англійською мовою).\nУдачі у відгадуванні.");
const guessTheWord = "lemonade".toLowerCase();
function translator(word) {
    let indexOfWord = 0;
    let guessWord = 0;
    function letterOfWord() {
        let userAttempt = prompt(`Введіть літеру загадуваного слова. Літера№${indexOfWord + 1}`, "A")?.toLowerCase();
...
document.write(`<p>Кількість вгаданих літер = ${finalScore}, загадане слово: ${guessTheWord}</p>`);
