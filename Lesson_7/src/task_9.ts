...
function countNegativeTemperature(numbersOfShows: number): string {
  let count = 0;
  let userTemperature: number = 0;
  let allTemperature = "";

  for (let i = 1; i <= numbersOfShows; i++) {
    userTemperature = parseInt(prompt(`Enter your ${i} temperature`, "10")!);
    if (userTemperature < 0) count++;
    allTemperature += `${userTemperature}`;
    if (i < numbersOfShows) allTemperature += ", ";//прибираємо кому після останнього символу 
  }
