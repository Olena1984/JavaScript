...
class Form {
  ...
  getValue() {
    const inputEl = this.inputForm.querySelector("input") as HTMLInputElement;
    const inputValue = String(inputEl.value);
    return inputValue;
  }
  setValue(newValue: string) {
    let inputEl = this.inputForm.querySelector("input") as HTMLInputElement;
  ...
  render() {
    ...
    if (container) container.append(this.inputForm);
  }
}

class Student {
  constructor(
    public formMonth: Form,
    public formScore: Form,
  ) {}
  checkMonth() {
    const userMonth = this.formMonth.getValue();
    const numberOfMonth = Number(userMonth);
    try {
      if (userMonth === "") throw new notFilledFieldError();
      if (isNaN(numberOfMonth)) throw new isNotNumberError();
  ...
  checkScore() {
    ...
    try {
      if (userScore === "") throw new notFilledFieldError();
  ...
  ImproveScore() {
    ...
    const parentEl = document.querySelector(".container") as HTMLDivElement;
    const divEl = document.createElement("div");
