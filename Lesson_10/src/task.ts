...
let securitiesArr = Array.from({ length: 25 }, () =>
  Math.floor(1 + Math.random() * (10000 - 1 + 1)),
);
...
// reduce
let listArrMorePrev = securitiesArr.reduce(
  (initialValue: number[], currentNumber: number,index:number, array:number[]) => {
...
let maxVal = securitiesArr.reduce((initialValue: number, number: number) => {
...
// map
let arrPricesAsPercentage = securitiesArr.map(
  (number: number) => (number / maxVal) * 100,
);
...
// reduce
let countChanges = securitiesArr.reduce(
  (countValue: number, number: number, index: number, array: number[]) => {
...
// some
let priceLess1000 = securitiesArr.some((number: number) => number < 1000);
...
// every
let pricesMore1000 = securitiesArr.every((number: number) => number > 1000);
console.log(pricesMore1000);

// ===============================================
//8)Підрахувати кількість цін, що більше за 1000
// reduce
let countPricesMore1000 = securitiesArr.reduce(
  (countInitial: number, number: number) =>
    number > 1000 ? countInitial + 1 : countInitial,
  0,
);
console.log(countPricesMore1000);
// ================================================
//9)Підрахувати суму цін, що більше за 1000
let countSumPricesMore1000 = securitiesArr.reduce(
  (sum: number, number: number) => (number > 1000 ? sum + number : sum),
  0,
);
...
// find
let firstPriceMore1000 = securitiesArr.find((number: number) => number > 1000);
...
let indexPriceMore1000 = securitiesArr.findIndex(
...
let lastPriceMore1000 = securitiesArr.findLast(
  (number: number) => number > 1000,
);
...
let indexLastPriceMore1000 = securitiesArr.findLastIndex(
  (number: number) => number > 1000,
);
